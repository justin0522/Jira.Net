...
namespace Jira.Net.Models
{
	...
	public class Sprint
	{
        ...
        [DataMember(Name = "linkedPagesCount")]
		...
		public List<Issue> GetIssues()
		{
			if (_issues == null)
			{
				_issues = _jira.Client.GetIssuesFromSprint(this.ID.Value).Issues;
		...
		public List<User> GetAssignableUsers(List<Issue> additionalIssues, DateTime from, DateTime until)
		{
			if (_users == null)
			{
				...
				_users.ForEach(user =>
				{
					...
					user.IsProjectLead = projects.Any(project => project.Lead.Equals(user));
